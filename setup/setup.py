...
def modify_start_startup_win(launch_start):
    ...
    if launch_start:
        try:
            destination_folder = os.path.join('C:\\Users', os.getlogin(),
        ...
        windows_function(parent_folder, True)
    print(SUCCESS_MESSAGE, flush=True)
    update_toml("launch_start", str(not launch_start))
...
def update_toml(params: str, new_value: str | bool, debug: bool = False) -> None:
    """Update TOML file from params dictonary.

    Args:
        params (str): _description_
        new_value (str): _description_
        debug(bool): is a debug parameter
    """
...
def show_settings(*args, **kwargs):
    ...
    _, launch_start, defaul_start, display_console = get_data()
    mss_list = ["The option for the launch start with the system is: ", "The option for the default launch is: ",
                "The option for the show of display is: ", ]
    value = [launch_start, defaul_start, display_console]
    print(colorama.Fore.BLUE + colorama.Style.BRIGHT + "\n\t    ------ [SETTINGS] -------\n", flush=True)
    for mss in mss_list:
        print(colorama.Fore.CYAN + colorama.Style.BRIGHT + " - " + mss + colorama.Fore.GREEN + str(
            value[mss_list.index(mss)]), flush=True)
...
def first_start(cli, parent_folder):
    ...
    choise = take_value("(-) 2. Do you want to specify a default interface type? (default no) (Y/N): ", ('Y', 'N'))
    if choise == 'Y':
